...
pub async fn restart_one(app_handle: AppHandle, project_id: Uuid, task_set_id: Uuid) -> Result<()> {
    // TODO error handling is not correct, because this will run in a thread its not possible to proagate the errors with ? to the user. So it should set the whole session into failed
